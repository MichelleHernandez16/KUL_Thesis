...
df <- read_csv("premier_simple.csv", show_col_types = FALSE)
injuries_df <- read_csv("players_injuries_detailed.csv", show_col_types = FALSE)
career_df <- read_csv("premier_career_history_full.csv", show_col_types = FALSE)
...
surv_df <- df %>%
  select(
    ...
    fbref_url = fbref_url, #NEW:For time dependence 
    transfer_url = transfermarkt_url, #NEW:For time dependence
    career_length,
    retired,
    retirement_age,
    age_current,
    ...
    games_missed_total,
    total_matches_club,
    total_matches_total, #Needed for time-dependence (club+nationalteam)
    seasons_club,         # NEW: Added for rate calculations
    foot = foot.x,        # NEW: Extracted preferred foot
    position_simple,
    height = height.y
  ) %>%
  mutate(
